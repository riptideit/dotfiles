...
if [ -f /usr/share/zsh-autosuggestions/zsh-autosuggestions.zsh ]; then
  ...
  print -P "%F{yellow}[zsh] autosuggestions plugin not found%f"
fi
...
if [ -f /usr/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh ]; then
  ...
  print -P "%F{yellow}[zsh] syntax-highlighting plugin not found%f"
fi

# Ensure styles array exists even if plugin defines it late
...
if command -v tmux &> /dev/null; then
  # Only auto-start if we're not already inside tmux
  if [ -z "$TMUX" ]; then
